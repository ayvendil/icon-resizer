...
figma.ui.onmessage = async (msg) => {
  ...
  // Добавить префикс к выделенным иконкам по их размеру
  if (msg.type === "add-prefix") {
    const nodes = figma.currentPage.selection;
    if (nodes.length === 0) {
      figma.ui.postMessage({ type: "error", text: "Select icons first" });
      return;
    }
    ...
    for (const node of nodes) {
      ...
      const preset = presets.find(p => p.size === w);
      const baseName = node.name.split("/").pop().trim();

      if (preset) {
        const prefix = preset.prefix !== undefined ? preset.prefix : preset.size + "/";
        node.name = prefix + baseName;
      } else {
        // Если размер не найден в пресетах — просто оставляем как есть
        node.name = baseName;
      }
      count++;
    }
  ...
  // Detach instances and convert to components
  if (msg.type === "detach-and-make-components") {
    ...
    for (const node of nodes) {
      ...
      component.x = target.x;
      component.y = target.y;
      component.resize(target.width, target.height);
